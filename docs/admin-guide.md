...
| `NBI_ACP_AGENT_COMMAND`                          | str  | unset                       | env                                          | Overrides the adapter command NBI launches for the selected ACP agent type (shell-split). For Codex, NBI appends its `-c` approval and sandbox options, so a replacement must pass them through to codex-acp. The command starts in NBI's user directory (`~/.jupyter/nbi`), not in the JupyterLab root, so name it by absolute path. Unset uses the agent's registered default command. See [Gating the experimental ACP agent](#gating-the-experimental-acp-agent-378).                                                                                                                                                                                                                                                                         |

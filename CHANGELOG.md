...
- **Markdown an agent writes into a notebook cell no longer fetches remote media**. A markdown cell added or rewritten by the agent renders as soon as it lands and again whenever the notebook is opened, so an image in it was requested by the browser with no click, with a target and query string the model chose. Images in agent-written cells are now inserted as links, and raw HTML elements that fetch when rendered are inserted as visible text, matching how the chat sidebar already renders model markdown without fetching image sources. Code blocks and inline code are left as written.

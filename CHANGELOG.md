...
- **The MCP config editor no longer copies credentials into the workspace.** The Settings dialog's Add / Edit button opens the MCP config in the file editor by way of a temporary file under the Jupyter root, and that copy carried every `headers` and `env` value in plaintext with ordinary file permissions, in the directory the agent modes read from, until the editor tab was closed. The server now serves those values as `<redacted>` and restores the stored value when the edit is saved; leaving the placeholder in place keeps the credential, typing over it replaces it, and a placeholder with nothing stored behind it is refused rather than saved.

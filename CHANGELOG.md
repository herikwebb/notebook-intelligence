...
- **The ACP adapter no longer starts inside the workspace.** ACP mode launches its adapter with `npx -y <package>`, and npm decides what that runs from its current directory: a `node_modules/<package>` already there that satisfies the pinned version is executed instead of the registry release, and an `.npmrc` found by walking up from the directory picks the registry the package is fetched from. The adapter was started in the JupyterLab root, so a checkout opened as the root could choose what ran on session creation, before any approval prompt. The adapter now starts in NBI's own user directory; the workspace still reaches the agent as the explicit `cwd` of `session/new` and `session/load`, and the `nbi_workspace_root` MCP tool reads the root from an environment variable NBI sets on the server entry rather than from its cwd. A command set through `NBI_ACP_AGENT_COMMAND` is resolved from that directory too, so name it by absolute path.

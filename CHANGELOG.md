...
- **The file tools no longer follow a symlink swapped in after the workspace check.** `read_file`, `insert_content`, and `write_to_file` confine a path to the workspace and then re-opened it by name, so a file or directory component replaced by a symlink between the two resolutions was followed, and a write meant for the workspace could land on any file the server user can write. The tools now open with `O_NOFOLLOW`, compare the path the kernel actually opened against the confined one before truncating, and refuse the call on a mismatch.

...
- **Chatbook `@` mentions no longer follow symlinks**. A mention token that named a hidden file or a skipped directory was refused, but one that reached the same place through a symlink inside the workspace was resolved and its content sent to the model. Mentions now refuse any path with a symlinked component, matching the picker, which never offered one, and re-check the hidden and skipped rules against the resolved path.

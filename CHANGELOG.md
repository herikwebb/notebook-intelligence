...
- **stdio MCP servers start in NBI's user directory**. A server listed in `mcp.json` inherited the Jupyter server's working directory, which is the JupyterLab root when JupyterLab is started from a checkout. Launchers such as `npx` and `python -m` resolve what they run from their working directory, and NBI starts every configured server when the extension loads, so a checkout opened as the root could decide what ran as the server. Servers now start in `~/.jupyter/nbi`; a relative `command` or path argument in `mcp.json` resolves there rather than wherever JupyterLab happened to be started. The same change was already made for the ACP adapter.

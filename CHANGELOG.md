...
- **Ask-mode chat requests now fit prior conversation and current context to the active model's input budget.** System instructions and the newest user prompt are preserved, current-turn context is prioritized, and older history is retained only as complete turns so oversized sessions degrade predictably instead of overflowing the model window. Agent tool loops remain unchanged.

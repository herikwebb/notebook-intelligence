...
- **`NBI_CLAUDE_CODE_TOOLS_POLICY=force-off` now reaches the agent.** The policy stripped the Claude Code built-in tool set from the capabilities response and the config write filter, but the options handed to the Claude Agent SDK never consulted that list, so the CLI still started with Bash, Edit, Write, WebFetch and the rest under an admin lock. The client now passes an empty built-in tool base set (or, on an SDK without that option, a disallow list of the built-in tools) whenever the grant is absent; MCP-served Jupyter UI tools are unaffected.

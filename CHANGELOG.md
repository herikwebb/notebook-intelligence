...
- **Ask-mode chat requests now fit prior conversation and current context to the active model's input budget.** System instructions and the newest user prompt are prioritized, with the prompt truncated only when mandatory context cannot otherwise fit; current-turn context comes next, and older history is retained only as complete turns. Agent tool loops remain unchanged. OpenAI-compatible and LiteLLM-compatible providers require an explicit **Context window** setting; a blank setting leaves history unchanged rather than pruning against a guessed limit.

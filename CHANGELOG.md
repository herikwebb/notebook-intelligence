...
- **Ask-mode chat requests now fit prior conversation and current context to the active model's input budget.** System instructions and the newest user prompt are prioritized, with the prompt truncated only when mandatory context cannot otherwise fit; current-turn context comes next, and older history is retained only as complete turns. Agent tool loops remain unchanged.
- **MCP 1.x compatibility is enforced during installation.** The Python dependency now excludes MCP 2.x, which removed the `mcp.server.fastmcp` import path used by the extension and caused fresh installations to fail extension validation.

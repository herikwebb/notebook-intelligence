...
def _chat_history_context_window(chat_model) -> int:
    """Skip history pruning when a compatible provider only guessed a size."""
    if getattr(chat_model, "context_window_is_configured", True) is False:
        return 0
    return chat_model.context_window
